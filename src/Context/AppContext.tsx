...
const AppContext = React.createContext<IMealApiResponse | null>(null);

const AppProvider = ({ children }: { children: ReactNode }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [searchTerm, setSearchTerm] = useState<string>("a");
  const [meals, setMeals] = useState<IMealApiResponse["meals"] | {}>({});
  ...
  //       const { meals } = data;
  //       if (meals) {
  //         const newMeals = meals.map((item: IMealApiResponse) => {
